...
async function fts_get(tid, tarea, which) {
  let e = W.jq(tid)()
  let txt = W.jq(tarea)
  let query = txt()
  if (e=="?"){query=e}
...
function main(){
  ...
  act.show = (v,m)=>{
    ...
    else if (v=="fts" || v=="tmrec"){m.m.disp("none");m.i.disp("inline");m.b.disp("none");m.t.disp("inline")}
    else {m.m.disp("inline");m.i.disp("none");m.b.disp("none");m.t.disp("none")}
  }
  act.bpll = ()=>{W.hxdom("get", "/cgi-bin/lude.cgi/memo", "sel", "#hnt")}
  act.bpsh = push_memo
  act.bfts_get = ()=>{fts_get("tid","txt", act["act"])}
  act.bfts_set = ()=>{fts_set("tid","txt", act["act"])}
